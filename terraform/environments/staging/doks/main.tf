# Ambiente de Staging - DOKS
# Réplica de producción con recursos reducidos para pruebas finales
...
module "doks_cluster" {
  ...
  # Node Pool Configuration - Intermedio entre dev y prod
  ...
  tags = concat(
    var.tags,
    ["environment:staging", "terraform:true"]
  )
  node_labels = merge(
    var.node_labels,
    {
  ...
  # Container Registry - Puede compartirse con prod
