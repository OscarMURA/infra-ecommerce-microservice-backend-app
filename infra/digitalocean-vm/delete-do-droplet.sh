...
#  - curl, jq
# Usage:
#  DO_TOKEN=... NAME=ecommerce-integration-runner ./delete-do-droplet.sh
#  DO_TOKEN=... DROPLET_ID=123456 ./delete-do-droplet.sh
...
if [[ -z "${DROPLET_ID:-}" ]]; then
  echo "Resolving droplet ID by name: ${NAME}"
  PAGE=1
  while :; do
    RESP=$(curl -sS -H "Authorization: Bearer ${DO_TOKEN}" "https://api.digitalocean.com/v2/droplets?page=${PAGE}&per_page=200")
    ...
    if [[ -z "$NP" || "$NP" == "null" ]]; then
      if [[ "$ALLOW_MISSING" == "1" ]]; then
        echo "Droplet with name '$NAME' not found, skipping delete."
        exit 0
      else
...
elif [[ "$HTTP_CODE" == "404" && "$ALLOW_MISSING" == "1" ]]; then
  echo "Droplet ${DROPLET_ID} already absent, nothing to delete."
else
